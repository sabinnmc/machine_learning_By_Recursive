...
class TinyYolov4 : public YoloPost
{
public:
    TinyYolov4(HailoROIPtr roi, YoloParams *params)
        : YoloPost(params->labels, params->detection_threshold, params->iou_threshold, params->max_boxes), _tensors(roi->get_tensors())
    {
        if (_tensors.size() > 0)
        {
...
public:
        ...
        : YoloPost(params->labels, params->detection_threshold, params->iou_threshold, params->max_boxes), _roi(roi)
    {
        ...
        {
            hailo_format_type_t format;
            m_image_width = _roi->get_tensor("yolox_s_leaky/conv82")->width() * 32;
            m_image_height = _roi->get_tensor("yolox_s_leaky/conv82")->height() * 32;

            format = _roi->get_tensor("yolox_s_leaky/conv82")->vstream_info().format.type;
            _layers.push_back(std::make_shared<YoloXOL>(_roi->get_tensor("yolox_s_leaky/conv82"), _roi->get_tensor("yolox_s_leaky/conv83"),
                                                        _roi->get_tensor("yolox_s_leaky/conv81"), params->label_offset, format == HAILO_FORMAT_TYPE_UINT16));
            
            format = _roi->get_tensor("yolox_s_leaky/conv69")->vstream_info().format.type;
            _layers.push_back(std::make_shared<YoloXOL>(_roi->get_tensor("yolox_s_leaky/conv69"), _roi->get_tensor("yolox_s_leaky/conv70"),
                                                        _roi->get_tensor("yolox_s_leaky/conv68"), params->label_offset, format == HAILO_FORMAT_TYPE_UINT16));
            
            format = _roi->get_tensor("yolox_s_leaky/conv55")->vstream_info().format.type;
            _layers.push_back(std::make_shared<YoloXOL>(_roi->get_tensor("yolox_s_leaky/conv55"), _roi->get_tensor("yolox_s_leaky/conv56"),
                                                        _roi->get_tensor("yolox_s_leaky/conv54"), params->label_offset, format == HAILO_FORMAT_TYPE_UINT16));
            params->check_params_logic(get_num_classes());
        }
    };
    ~YoloX() = default;
...
class TinyYoloX : public YoloPost
{
public:
    TinyYoloX(HailoROIPtr roi, YoloParams *params)
        : YoloPost(params->labels, params->detection_threshold, params->iou_threshold, params->max_boxes), _roi(roi)
    {
        if (_roi->has_tensors())
        {
            hailo_format_type_t format;
            m_image_width = _roi->get_tensor("yolox_tiny/conv82")->width() * 32;
            m_image_height = _roi->get_tensor("yolox_tiny/conv82")->height() * 32;

            format = _roi->get_tensor("yolox_tiny/conv82")->vstream_info().format.type;
            _layers.push_back(std::make_shared<YoloXOL>(_roi->get_tensor("yolox_tiny/conv82"), _roi->get_tensor("yolox_tiny/conv83"),
                                                        _roi->get_tensor("yolox_tiny/conv81"), params->label_offset, format == HAILO_FORMAT_TYPE_UINT16));

            format = _roi->get_tensor("yolox_tiny/conv69")->vstream_info().format.type;
            _layers.push_back(std::make_shared<YoloXOL>(_roi->get_tensor("yolox_tiny/conv69"), _roi->get_tensor("yolox_tiny/conv70"),
                                                        _roi->get_tensor("yolox_tiny/conv68"), params->label_offset, format == HAILO_FORMAT_TYPE_UINT16));

            format = _roi->get_tensor("yolox_tiny/conv55")->vstream_info().format.type;
            _layers.push_back(std::make_shared<YoloXOL>(_roi->get_tensor("yolox_tiny/conv55"), _roi->get_tensor("yolox_tiny/conv56"),
                                                        _roi->get_tensor("yolox_tiny/conv54"), params->label_offset, format == HAILO_FORMAT_TYPE_UINT16));
            params->check_params_logic(get_num_classes());
        }
    };
    ~TinyYoloX() = default;
...
void tiny_yolov4(HailoROIPtr roi, void *params_void_ptr)
{
	YoloParams *params = reinterpret_cast<YoloParams *>(params_void_ptr);
	auto post = TinyYolov4(roi, params);
	auto detections = post.decode();
	hailo_common::add_detections(roi, detections);
}
...
void tiny_yolox(HailoROIPtr roi, void *params_void_ptr)
{
	YoloParams *params = reinterpret_cast<YoloParams *>(params_void_ptr);
	auto post = TinyYoloX(roi, params);
	auto detections = post.decode();
	hailo_common::add_detections(roi, detections);
}
...
YoloParams *init(const std::string config_path, const std::string function_name)
{
    ...
    if (!fs::exists(config_path))
    {
        ...
        else if (function_name.std::string::compare("yolov4tiny") == 0)
        {
            params = new Yolov4TinyParams;
        }
        else
        {
